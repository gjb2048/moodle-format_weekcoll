 ...
 * @namespace
 */
M.format_weekcoll = M.format_weekcoll || {};
 ...
 */
M.format_weekcoll.init = function(Y, wwwroot, moodleid, courseid, cookielifetime) {
    // Init.
    ourYUI = Y;
    thewwwroot = wwwroot;
...
M.format_weekcoll.set_current_week = function (Y, theWeek) {
    currentWeek = theWeek;
}
...
function toggleexactweek(target,image,toggleNum,reloading)  // Toggle the target tr and change the image which is the a tag within the td of the tr above target
{
    if(document.getElementById)
    {
        ...
        //alert("toggleexactweek tdisp:" + target.style.display + " disp:" + displaySetting + " ton:" + toggleNum + " rl:" + reloading);
        ...
        if (target.style.display == displaySetting)
        {
            ...
            image.style.backgroundImage = "url(" + thewwwroot + "/course/format/weekcoll/images/arrow_down.png)";
            // Save the toggle!
        ...
        else
        {
            ...
            image.style.backgroundImage = "url(" + thewwwroot + "/course/format/weekcoll/images/arrow_up.png)";
            // Save the toggle!
...
// Get the cookie - yum.
function restoreweekcollcookie(daYUI)
{
    return daYUI.Cookie.getSub("mdl_cf_weekcoll",thecookiesubid); 
}
...
M.format_weekcoll.reload_toggles = function (Y, aToggle) {
    numToggles = aToggle;
...
// Show a specific topic - used when in 'Show week x' mode.
M.format_weekcoll.show_week = function (Y, theWeek)
{
